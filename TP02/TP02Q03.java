...
public class TP02Q03 {
    ...
    public static Boolean procura(String namePesq){
        Boolean achei = false;
        for(int i = 0; i < totalPersonagens; i++){
            if (namePesq.equalsIgnoreCase(personagens[i].name)){
                return true;
            }
        }
        return achei;
    ...
    public static void main(String[] args) {
            try (BufferedReader br = new BufferedReader(new InputStreamReader(System.in, StandardCharsets.UTF_8))) {
                ...
                String procurando;
                while (!(procurando = br.readLine()).equalsIgnoreCase("FIM")) {
                    Boolean achado = procura(procurando.trim());
