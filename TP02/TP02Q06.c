...
int main() {
    ...
    ordenaSelecao(&personagens_encontrados, contEncontrados, 0);
...
void ordenaSelecao(Personagem** personagens_encontrados, int contEncontrados, int indice){
    //Verifica se puxou todos
    // for (int i = 0; i < contEncontrados; i++) {
    //     printf("%s\n", personagens_encontrados[i]->id);
    // }
    Personagem* tmp;
    if (indice == contEncontrados-1)
    return;

    int menor = indice;
    for (int i = indice + 1; i < contEncontrados; i++){
        if (strcmp(personagens_encontrados[i]->name, personagens_encontrados[menor]->name) < 0){
            menor = i;
        }
    }

    tmp = personagens_encontrados[indice];
    personagens_encontrados[indice] = personagens_encontrados[menor];
    personagens_encontrados[menor] = tmp;
    
    ordenaSelecao(personagens_encontrados, contEncontrados, indice +1);
}
