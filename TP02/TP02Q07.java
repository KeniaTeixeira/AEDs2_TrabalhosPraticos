...
public class TP02Q07 {
    ...
    public static void ordenando(){
        ...
        for(int i=0; i < totalPersonagens; i++){
            tmp = personagens[i];
            j = i -1;
            //se for menor que zero tmpDate vem antes de personagem
            while (j>=0 && (tmp.dateOfBirth.compareTo(personagens[j].dateOfBirth) < 0 || (tmp.dateOfBirth.compareTo(personagens[j].dateOfBirth) == 0 && (tmp.name.compareTo(personagens[j].name)) < 0))) {
