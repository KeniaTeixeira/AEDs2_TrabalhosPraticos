...
public class counting {
    ...
    public static void ordenando() {
        ...
        for (int i = 0; i < totalPersonagens; i++) {
            int anoNascimento = personagens[i].getYearOfBirth();
        ...
        for (int i = 0; i < totalPersonagens; i++) {
            int indice = personagens[i].getYearOfBirth() - menorAno;
            contagem[indice]++;
        }
        ...
        Personagens[] ordenado = new Personagens[totalPersonagens];
        for (int i = totalPersonagens - 1; i >= 0; i--) {
            int indice = personagens[i].getYearOfBirth() - menorAno;
            ordenado[contagem[indice] - 1] = personagens[i];
            contagem[indice]--;
        }
        ...
        for (int i = menorAno; i <= maiorAno; i++) {
            ...
            for (int j = 0; j < totalPersonagens; j++) {
                if (personagens[j].getYearOfBirth() == i) {
                    if (inicio == -1) {
                        inicio = j;
                    }
                } else if (inicio != -1) {
                    Arrays.sort(personagens, inicio, j, Comparator.comparing(Personagens::getName));
                    inicio = -1;
                }
            }
            if (inicio != -1) {
                Arrays.sort(personagens, inicio, totalPersonagens, Comparator.comparing(Personagens::getName));
            }
        }
