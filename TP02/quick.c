...
int main() {
   ...
   // verifica se pegou todos
    ...
    ordenaquick(&personagens_encontrados, 0,  contEncontrados-1);
...
int particionar(Personagem** persona, int inicio, int fim){
    Personagem* tmp;
    Personagem* pivo = persona[fim];
    int i = (inicio-1);

    for (int j = inicio; j <= fim -1 ; j++){
        if (strcmp(persona[j]->house, pivo->house) < 0 || (strcmp(persona[j]->house, pivo->house) == 0 && strcmp(persona[j]->name, pivo->name) < 0)){
            i++;
            tmp = persona[i];
            persona[i] = persona[j];
            persona[j] = tmp;
    ...
    tmp = persona[i+1];
    persona[i+1] = persona[fim];
    persona[fim] = tmp;
    
    return(i+1);
}

void ordenaquick(Personagem** personagens_encontrados, int inicio, int fim){
    Personagem* tmp;

    if (inicio < fim){
        int pi = particionar(personagens_encontrados, inicio, fim);

        ordenaquick(personagens_encontrados, inicio, pi-1);
        ordenaquick(personagens_encontrados, pi+1, fim);
    }
