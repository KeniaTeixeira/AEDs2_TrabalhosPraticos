...
int main() {
   ...
   // verifica se pegou todos
    ...
    ordenaShell(&personagens_encontrados, contEncontrados);
...
void ordenaShell(Personagem** personagens_encontrados, int contEncontrados){
    ...
    int intervalo;
    Personagem* tmp;
    while (intervalo < contEncontrados/3){
        intervalo = intervalo*3+1;
    }
    
    while (intervalo >= 1){
        for (int i = intervalo; i < contEncontrados; i++){
            tmp = personagens_encontrados[i];
            int j = i;
            while (j>= intervalo && strcmp(personagens_encontrados[j-intervalo]->eyeColour,tmp->eyeColour) > 0 || (j>= intervalo && strcmp(personagens_encontrados[j-intervalo]->eyeColour,tmp->eyeColour) == 0 && strcmp(personagens_encontrados[j-intervalo]->name, tmp->name)>0)){
                
            
            // while (j>= intervalo && strcmp(personagens_encontrados[j-intervalo]->eyeColour, tmp->eyeColour) < 0 || (strcmp(personagens_encontrados[j-intervalo]->eyeColour, tmp->eyeColour) == 0)&& (strcmp(personagens_encontrados[j-intervalo]->name, tmp->name) > 0)){
                personagens_encontrados[j] = personagens_encontrados[j-intervalo];
                j-= intervalo;
            }
            personagens_encontrados[j] = tmp;
        }
        intervalo /= 3;
    }
}
